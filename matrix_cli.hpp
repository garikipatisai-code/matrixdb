...
inline int matrix_repl(std::istream& in, std::ostream& out, CPUMockEngine& eng) {
    ...
    while (std::getline(in, raw)) {
        ...
        if (cmd == ".quit" || cmd == ".exit") break;
            ...
            // atomic catalog snapshot (columns + names + string dictionaries); fail-soft (no abort).
            if (tk.size() < 2) { out << "Error: usage: .save <file>\n"; continue; }
            if (!eng.save_catalog(tk[1])) { out << "Error: could not save catalog to " << tk[1] << "\n"; continue; }
            ...
            if (!eng.load_catalog(tk[1])) { out << "Error: not a valid/compatible matrixdb snapshot: " << tk[1] << "\n"; continue; }
