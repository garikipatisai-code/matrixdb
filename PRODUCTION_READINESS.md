...
| NW-5 | No connection management (pooling, limits, timeouts, backpressure) | Can't handle real connection churn **[partial — NW-5: per-connection recv timeout (slowloris resistance)]** | P1 | M | partial |
...
| ID | Gap | Why | Sev | Effort | Local? |
|----|-----|-----|-----|--------|--------|
| OB-1 | No structured logging (just `cout`) | Can't diagnose production issues **[FIXED — OB-1: leveled Log + runtime threshold]** | P1 | S | yes |
...
*OB-1 landed (structured logging, CPU): `logging.hpp` — a leveled `Log` (DEBUG<INFO<WARN<ERROR) with a runtime-settable threshold (default WARN), writing `[LEVEL] msg` to stderr (logs are not data → stdout stays clean). `Log::enabled(l)` lets callers skip building an expensive message below threshold. The engine's one genuinely-diagnostic message — the dropped-write (KVStore-full = data loss) path in the destructor — now logs at **ERROR** (was an unconditional `cout`), and `set_log_level()`/`log_level()` expose the knob (OB-4 synergy). Informational `cout` (init/shutdown) left as-is. test_logging.cpp (the threshold filter at WARN/DEBUG/ERROR; actual emission verified by capturing `cerr`'s rdbuf — below-threshold suppressed, WARN/ERROR emitted with their prefix). 59-test suite + oracle green. Deferred: a structured/JSON sink, per-subsystem levels, routing the informational `cout` through INFO, a wire op to set the level remotely.*
